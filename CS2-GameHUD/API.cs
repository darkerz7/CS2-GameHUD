...
namespace CS2_GameHUD
{
	internal class API : IGameHUDAPI
	{
		public void Native_GameHUD_SetParams(CCSPlayerController Player, byte channel, Vector vec, System.Drawing.Color color, int fontsize, string fontname, float units, PointWorldTextJustifyHorizontal_t justifyhorizontal, PointWorldTextJustifyVertical_t justifyvertical, PointWorldTextReorientMode_t reorientmode, float bgborderheight, float bgborderwidth)
		{
			...
			GameHUD.g_HUD[Player.Slot].Channel[channel].Params((System.Numerics.Vector3)vec, color, fontsize, fontname, units, justifyhorizontal, justifyvertical, reorientmode, bgborderheight, bgborderwidth);
		}
		...
		{
			if (!Player.IsValid || channel < 0 || channel >= GameHUD.MAXHUDCHANNELS) return;
			GameHUD.g_HUD[Player.Slot].Channel[channel].Params(vec3, color, fontsize, fontname, units, justifyhorizontal, justifyvertical, reorientmode, bgborderheight, bgborderwidth);
		}
		...
		{
			if (!Player.IsValid || channel < 0 || channel >= GameHUD.MAXHUDCHANNELS) return;
			GameHUD.g_HUD[Player.Slot].Channel[channel].UpdateParams((System.Numerics.Vector3)vec, color, fontsize, fontname, units, justifyhorizontal, justifyvertical, reorientmode, bgborderheight, bgborderwidth);
		}
		...
		{
			if (!Player.IsValid || channel < 0 || channel >= GameHUD.MAXHUDCHANNELS) return;
			GameHUD.g_HUD[Player.Slot].Channel[channel].UpdateParams(vec3, color, fontsize, fontname, units, justifyhorizontal, justifyvertical, reorientmode, bgborderheight, bgborderwidth);
		}
